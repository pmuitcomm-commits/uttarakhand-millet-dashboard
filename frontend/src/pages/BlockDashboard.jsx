...
function formatApiError(error) {
  ...
  return detail || error?.message || "Unable to update block officer details.";
}
...
function BlockOfficerDetailsCard() {
  ...
  useEffect(() => {
    ...
    async function loadDetails() {
      ...
      try {
        const response = await getBlockOfficerDetails();
        if (!mounted) return;
  ...
  const saveDetails = async () => {
    ...
    try {
      const response = await updateBlockOfficerDetails({
